// ИСПРАВЛЕННЫЙ КОД - сохраняет работу успешной отправки, но убирает ложные срабатывания
...
document.addEventListener('DOMContentLoaded', function() {
    // ИСПРАВЛЕНО: Переменные для отслеживания состояния
    let isElementorFormSubmission = false;
    let lastClickTime = 0;
    ...
    // ИСПРАВЛЕНО: Функция для показа модального окна
    ...
    // ИСПРАВЛЕНО: Отслеживаем клики по кнопкам отправки форм
    document.addEventListener('click', function(e) {
        const submitButton = e.target.closest('.elementor-button[type="submit"], .elementor-form button[type="submit"]');
        if (submitButton) {
            const form = submitButton.closest('.elementor-form');
            if (form) {
                console.log('Клик по кнопке отправки формы');
                isElementorFormSubmission = true;
                lastClickTime = Date.now();
                lastClickTarget = 'form-submit';
                
                // Сбрасываем флаг через 10 секунд на случай, если что-то пойдет не так
                setTimeout(() => {
                    isElementorFormSubmission = false;
                }, 10000);
            }
        } else {
            // ИСПРАВЛЕНО: Отслеживаем клики НЕ по формам (лайки, навигация и т.д.)
            const isReactionClick = e.target.closest('.post-reactions, .like-btn, .dislike-btn, .reaction-item');
            const isNavigationClick = e.target.closest('a, .nav, .menu, .navigation');
            
            if (isReactionClick || isNavigationClick) {
                lastClickTime = Date.now();
                lastClickTarget = isReactionClick ? 'reaction' : 'navigation';
                console.log('Клик по', lastClickTarget);
            }
        }
    });

    // Находим формы и добавляем обработчики
    const forms = document.querySelectorAll('.elementor-form');
    forms.forEach(form => {
        // Проверяем, есть ли уже сообщение об успешной отправке
        const successMessage = form.querySelector('.elementor-message-success');
        if (successMessage) {
            // Если сообщение уже есть, показываем наше модальное окно
            showSuccessPopup();
        ...
        form.addEventListener('submit', function(e) {
            ...
            // Устанавливаем таймер для проверки успешной отправки
            setTimeout(function() {
                const elementorPopup = document.querySelector('.dialog-widget.elementor-popup-modal');
    ...
    // ИСПРАВЛЕНО: Добавляем обработчик для отслеживания AJAX-запросов
    const originalXHR = window.XMLHttpRequest;
    window.XMLHttpRequest = function() {
        const xhr = new originalXHR();

        xhr.addEventListener('load', function() {
            // ИСПРАВЛЕНО: Проверяем контекст запроса
            const timeSinceLastClick = Date.now() - lastClickTime;
            const isRecentFormSubmission = isElementorFormSubmission && timeSinceLastClick < 5000;
            const isRecentReactionClick = lastClickTarget === 'reaction' && timeSinceLastClick < 2000;
            const isRecentNavigationClick = lastClickTarget === 'navigation' && timeSinceLastClick < 2000;
            
            if (this.responseText && this.responseText.includes('success')) {
                console.log('AJAX success response detected', {
                    isElementorFormSubmission,
                    lastClickTarget,
                    timeSinceLastClick,
                    isRecentFormSubmission,
                    isRecentReactionClick
                });
                
                // Показываем модальное окно только если это была отправка формы
                if (isRecentFormSubmission && !isRecentReactionClick && !isRecentNavigationClick) {
                    setTimeout(function() {
                        // Закрываем модальное окно Elementor
                        const elementorPopup = document.querySelector('.dialog-widget.elementor-popup-modal');
                        if (elementorPopup) {
                ...
                } else if (isRecentReactionClick || isRecentNavigationClick) {
                    console.log('Блокируем показ модального окна - это был клик по', lastClickTarget);
                }
            }
        });
    ...
    // ИСПРАВЛЕНО: MutationObserver с улучшенной логикой
            ...
            if (mutation.type === 'childList') {
                ...
                mutation.addedNodes.forEach(node => {
                    if (node.classList && node.classList.contains('elementor-message-success')) {
                        ...
                        console.log('Success message node added', {
                            isElementorFormSubmission,
                            lastClickTarget,
                            timeSinceLastClick,
                            isRecentFormSubmission,
                        ...
                        if (isRecentFormSubmission && !isRecentReactionClick) {
                            // Закрываем модальное окно Elementor
                            const elementorPopup = document.querySelector('.dialog-widget.elementor-popup-modal');
                            if (elementorPopup) {
                                const closeButton = elementorPopup.querySelector('.dialog-close-button');
                            ...
                            // Показываем наше кастомное модальное окно
                            showSuccessPopup();
                            ...
                            console.log('Блокируем показ модального окна при добавлении success node');
                        }
                    }
                });
                
                // Проверяем измененные атрибуты
                if (mutation.target.classList && mutation.target.classList.contains('elementor-message-success')) {
    ...
    // Начинаем наблюдение за изменениями в форме и её родителе
    forms.forEach(form => {
    ...
    // ВЕСЬ ВАШ СУЩЕСТВУЮЩИЙ КОД ДЛЯ ТЕЛЕФОНОВ ОСТАЕТСЯ БЕЗ ИЗМЕНЕНИЙ
    ...
    // Добавляем обработчик для модальных окон Elementor
        ...
        mutations.forEach(function(mutation) {
            if (mutation.type === 'childList') {
                const hasNewForms = Array.from(mutation.addedNodes).some(node =>
                    node.nodeType === 1 && (
                        node.querySelector('#form-field-tel') ||
                        (node.id === 'form-field-tel')
                    )
                );
